...
stitch_mets= set()
stitch_genes= set()
with open("511145.protein_chemical.links.v5.0.tsv", "r") as file:
...
with open('mets_final.csv', 'r') as file:
    ...
    for row in reader:
        if row[3] != '':
            mets_to_cid[row[0]]= row[3]
            model_cids.add(row[3])
...
target_genes= list(model_genes.intersection(stitch_genes))
