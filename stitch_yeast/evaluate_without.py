...
def make_dataset_exception(met_names, met_features, gene_names, gene_features, confidence_score, exceptions):
...
if cs == 150:
    shared_mpis_file= 'stitch_yeast_150_shared_mpis.csv'
elif cs == 400:
    shared_mpis_file= 'stitch_yeast_400_shared_mpis.csv'
elif cs == 700:
    shared_mpis_file= 'stitch_yeast_700_shared_mpis.csv'
elif cs == 900:
    shared_mpis_file= 'stitch_yeast_900_shared_mpis.csv'
else:
    sys.exit()

shared_mpis= set()
with open(shared_mpis_file, 'r') as file:
    reader= csv.reader(file, delimiter= ',')
    for row in reader:
        shared_mpis.add((row[0], row[1]))
print("Number of shared MPIs", len(shared_mpis))

X, y= make_dataset_exception(metabolites_names, metabolites_features, protein_names, protein_features, cs, shared_mpis)
